...
def generate_simulation_data(numberECUs, vulnProb, categoriesValues, maxVuln, minVuln,
                            adasWeight, powertWeight, hmiWeight, bodyWeight, chWeight, 
                            qmWeight, aWeight, bWeight, cWeight, dWeight):
    ...
    for _ in range(numberECUs):
            ...
            if random.random() < vulnProb:
                ...
                while len(vulnsWML) < 3: {vulnsWML.append(0)}
...
def vehicleRatingCalculus(values, categoriesValues, asil, cal, dp, iso, risk, cRating):         
    ...
    if (cRating >= 0 and cRating <= 5):
        # Class D
        if ((asil == "D" or asil == "C") or 
            ((asil == "B" or asil == "A") and (cal == 4 or cal == 3) and risk == "High")):
    ...
    with open(filename, mode="a", newline="", encoding="utf-8") as file:
        ...
        # Define header for the CSV file
            header = [
                ...
                "sum_D","n_Times_D","Avg_D","mean_D","std_D","sum_C","n_Times_C","Avg_C","mean_C","std_C","sum_B","n_Times_B","Avg_B", "mean_B","std_B","sum_A","n_Times_A","Avg_A", "mean_A","std_A","Final Rating"
...
def main():    
    ...
    if(type == "auto"): 
        ...
        for _ in range(n):
            ...
            data = generate_simulation_data(numberECUs, vulnProb, categoriesValues, maxVuln, minVuln,
                                            domainWeight[0], domainWeight[1], domainWeight[2], domainWeight[3], domainWeight[4],
                                            safetyWeight[0], safetyWeight[1], safetyWeight[2], safetyWeight[3], safetyWeight[4])
